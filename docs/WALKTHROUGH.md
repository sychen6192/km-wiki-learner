...
| 掃描 PDF、照片 | 自動偵測沒有文字層 → 轉圖 → OCR（或交給 vision 模型，見下） |
| `.docx` | 標準函式庫直接解 |

**掃描件建議用 vision 模型讀，不要用 OCR。** 版面密、字小、多語混排的教材，OCR 會吐出
看起來像文字的雜訊——而雜訊比空白危險：下游模型讀不懂就會自己補，補出一份煞有其事
但與課本無關的摘要。實測同一頁單字表，OCR 的譯文欄全是亂碼，vision 則逐條抓對了單字。

```bash
KM_VISION_MODEL=qwen3.8:27b KM_API_BASE=http://llm:11434 KM_RASTER_DPI=300 make extract
```

代價是慢：一頁約 1～4 分鐘，一本 20 頁的講義要跑上半小時到一小時。先用
`KM_VISION_MAX_PAGES=4` 試幾頁看品質再放開。連不上模型會自動退回 OCR，不會掉素材。

> ⚠️ 仍要抽查：**標音（ふりがな）字太小時，模型可能改用自己的日文知識生一個讀音出來**。
> 單字本身可信度高，讀音請對照課本再進複習卡。
