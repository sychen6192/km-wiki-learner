...
3. **來源潔癖。** 每個外部事實都要能指出出處，優先一手來源。查證不到就明說「不確定」，
   **絕不編造**。
   ⚠️ **練習題素材裡有一半的內容是故意寫錯的。** 選擇題的誘答、用法題的錯誤例句，
   都是設計來讓人選錯的。動筆前先找解答頁（`答え`／`解答`），對照出哪個才是正解；
   找不到解答就只記錄題目，**不要自己判斷哪個對**。把誘答當成教材內容寫進筆記，
   等於把錯的日文餵進複習卡。
4. **增量編輯。** 保留既有結構與人類手筆，讓 git diff 讀起來清爽有理。
   `locked: true` 的頁面完全不碰。
5. **不毀損複習進度。** 絕不修改或刪除 `<!--SR:...-->` 註解。
6. **自我驗收。** 交件前跑 `"$KM_PYTHON" tools/vault.py lint`，0 error 才算完成。
   （用 `$KM_PYTHON`，不要寫死 `python3`：在 Windows 上那個名字是個什麼都不做的空殼。）
   儀表板由 `stats` 生成，不要手改。
